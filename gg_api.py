...
potential_answers_to_ignore = ["@", "golden globe", "goldenglobe", "globe", "globes", "golden", "hosts", "http", "#", "-", "motion picture", "tv", "movie", "mejor", "/", "congrats", "wins", "film"]
...
def updateAnswerDictionary(year, hosts, awardNames, winners, presenters, nominees, best_dressed, answer):
    #Hosts
    ...
    for year in years:
        ...
        for x, col in tweets.iterrows():
            ...
            if matches != []:
                ...
                nominees_keys = ["nom", "award", "hope", "should", "wish", "chance", "could", "go", "love"]
                if ((any(i for i in winner_keys if(i in tweet.lower()))) or (any(i for i in presenter_keys if(i in tweet.lower()))) or (any(i for i in host_keys if(i in tweet.lower()))) or (any(i for i in nominees_keys if(i in tweet.lower())))):
                    doc = nlp(tweet)
                    count += 1
                else:
                    continue
                ...
                best_dressed_keys = ["suit", "dress", "cloth", "wear", "red carpet"]
        ...
        #Fill in final JSON
        #print(presenters)
        answer = updateAnswerDictionary(year, hosts, awardNames, winners, presenters, nominees, best_dressed, answer)
